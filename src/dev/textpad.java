...
public class textpad extends JFrame implements ActionListener,DocumentListener  
{   //菜单  
   ...
   //构造函数开始
   static String[][] TextList = new String[9][2];
   ...
   public textpad()
   {
       ...
       TextList[0][0]="PRTS联络2";
       TextList[4][0]="凯尔希";//讲述器不回溯
       TextList[6][0]="on";
       TextList[7][0]="老女人";
       TextList[8][0]="尸体在说话？（看到此界面代表系统初始化正常）";
       ...
       final String[] Linenum = {null};
       AtomicReference<Boolean> SocketFlag= new AtomicReference<>(false);
       ...
       editArea.addCaretListener(new CaretListener() {
		...
		public void caretUpdate(CaretEvent e) {
			// TODO 自动生成的方法存根
			int cx=editArea.getCaretPosition();
			try{
				String fronttext=editArea.getText(0, cx);
				int linenum=0;
				for(char i:fronttext.toCharArray()) {
				if(i=='\n')linenum++;
				}
                //获取行数
                //行数输出到文本
                ...
                bfw.write(Integer.toString(linenum-1));  
                bfw.flush();//刷新该流的缓冲  
                bfw.close();
                ...
                viewer.explainer.generateEvent();
                //输出到通信端口
//                showflag=true;


            }
            catch (BadLocationException ioException)  {
            }
			}
       });
   ...
   {   //新建  
       ...
       {   editArea.requestFocus();  
           JOptionPane.showMessageDialog(this,"此程序为YSP可预览编辑器，如需录制视频请使用YSP","帮助主题",JOptionPane.INFORMATION_MESSAGE);
       }//帮助主题结束  
       //关于  
   ...
   {   editMenu_Undo.setEnabled(true);  
    ...
    //解释核心
    public void Explain(int linenum){
  /*      TextList[0][0]="下北泽主场景";
        TextList[2][0]="野兽先辈の呐喊";//音效回溯1行
        TextList[3][0]="李姓男子";//讲述器不回溯
        TextList[6][0]="on";
        TextList[7][0]="？？？";
        TextList[8][0]="哼哼哼啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊";*/



        String[] texts = editArea.getText().split("\n");

        try{
            if(texts[linenum]==null | texts[linenum].equals("")){
                return;
            }
        }catch (Exception e){
            return;
        ...
        try {
            ...
            for (int i = linenum; i >= 0; i--) {
                if (texts[i].startsWith(">>>")) {
                    String[] talkers = removeEmpty(texts[i].split(">>>"));
                    if (talkers.length == 1) {
                        String[] tmpM = TalkHandler(talkers[0]);
                        TextList[4][0] = tmpM[0];
                        TextList[7][0] = tmpM[1];
                        TextList[8][0] = tmpM[2];
                        TextList[3][0] = null;
                        ...
                        String[] tmpL = TalkHandler(talkers[0]);
                        String[] tmpR = TalkHandler(talkers[1]);
                        if (tmpL[2] != null) {
                            TextList[7][0] = tmpL[1];
                            TextList[8][0] = tmpL[2];
                        } else {
        ...
        try {
            //音效
            if (texts[linenum].startsWith("SND") | texts[linenum].startsWith("sound")) {
                TextList[2][0] = removeEmpty(texts[linenum].substring(texts[linenum].indexOf('(') + 1, texts[linenum].indexOf(')')).split(","))[0];
            } else if (texts[linenum - 1].startsWith("SND") | texts[linenum - 1].startsWith("sound")) {
                TextList[2][0] = removeEmpty(texts[linenum - 1].substring(texts[linenum - 1].indexOf('(') + 1, texts[linenum - 1].indexOf(')')).split(","))[0];
            } else {
                TextList[2][0] = null;
            }
        }catch (Exception e){
            ...
            boolean BVGflag = false;
            boolean MSCflag = false;
            boolean CVRflag = false;

            for (int i = linenum; i >= 0; i--) {
                try {

                    if (!BVGflag && (texts[i].startsWith("BGP") | texts[i].startsWith("backdrop"))) {
                        TextList[0][0] = removeEmpty(texts[i].substring(texts[i].indexOf('(') + 1, texts[i].indexOf(')')).split(","))[0];
                        BVGflag = true;
                        continue;
                    }
                }catch (Exception e){
                ...
                try {
                    if (!MSCflag && (texts[i].startsWith("BGM") | texts[i].startsWith("music"))) {
                        TextList[1][0] = removeEmpty(texts[i].substring(texts[i].indexOf('(') + 1, texts[i].indexOf(')')).split(","))[0];
                        MSCflag = true;
                        continue;
                    }
                }catch (Exception e){
                ...
                try {
                    if (!CVRflag && (texts[i].startsWith("CVR") | texts[i].startsWith("cover"))) {
                        String line = removeEmpty(texts[i].substring(texts[i].indexOf('(') + 1, texts[i].indexOf(')')).split(","))[0];
                        if (line.equals("on") | line.equals("true")) {
                            TextList[6][0] = "on";
                        } else {
                            TextList[6][0] = "null";
                        }
    ...
    public String[] TalkHandler(String text){
        String avg;
        String name;
        String word;
        String textunc;
        if(text.charAt(text.length()-1)==')'){
            textunc = text.substring(0,text.indexOf('('));
        }else{
            textunc = text;
        }
        if(textunc.charAt(textunc.length()-1)==':'){
            word = null;
            String[] nameorigin = removeEmpty(textunc.split(":"));
            if(nameorigin.length>0){
                avg = removeEmpty(nameorigin[0].split("/"))[0];
                if (nameorigin.length>1){
                    name = nameorigin[1];
                }else{
                    name = avg;
                    if(textunc.charAt(0)==':'){
        ...
        return new String[]{avg, name, word};
    }
    public String[] removeEmpty(String[] origin){
        return Arrays.stream(origin).filter(s -> !"".equals(s)).toArray(String[]::new);
    }
